...
def get_page_of_search_results(request):
    if request.method != "GET":
        ...
        try:
            if (
                request.GET.get("keyword") == "undefined"
                or int(request.GET.get("keyword")) == 0
            ):
                return JsonResponse({"last_page": 0, "data": []})

            mongo_client = MongoConnection(
                str(os.getenv("MONGODB_DATABASE")), str(os.getenv("MONGODB_COLLECTION"))
            )
            ...
            if page_size not in (20, 30, 40, 50):
                return JsonResponse(
                    {
                        "error": "El tamaño de los resultados de búsqueda debe tener alguno de los siguientes valores: 20, 30, 40, 50"
                    },
                    status=400,
                )
            ...
            if page_number < 0:
                return JsonResponse(
                    {"error": "La página solicitada es inválida(Menor a 0)"}, status=400
                )
                    ...
                    return JsonResponse(
                        {
                            "last_page": 0,
                            "data": [],
                            "error": "Sin resultados de búsqueda  para esta keyword",
                        }
                    )

                return JsonResponse(
                    {"error": "La página solicitada es mayor a la última disponible"},
                    status=400,
                )
            ...
            data_dict = {"last_page": last_page, "data": search_results}
            return JsonResponse(data_dict, encoder=MongoJSONEncoder)
        except TypeError as e:
            print(e)
            return JsonResponse(
                {"error": "Debes proporcionar los parametros necesarios"}, 400
            )
...
def get_search_result_by_id(request, id):
    if request.method != "GET":
        ...
        try:
            ...
            if search_result:
                ...
                if search_result.get("urlAttach") is not None:
                    if len(search_result["urlAttach"]) >= 1:
                        first_attachment_url = search_result["urlAttach"][0][
                            "urlAttach"
                        ]
                        search_result["firstUrl"] = first_attachment_url

                        attachments_with_sinopsys = list(
                            filter(
                                lambda attachment: attachment["sinopsys"] != "",
                                search_result["urlAttach"],
                            )
                        )

                        attachments_with_bold_sinopsys = list(
                            map(
                                lambda attachment: {
                                    **attachment,
                                    "sinopsys": resaltar_keywords(
                                        subkeywords, attachment["sinopsys"]
                                    ),
                                },
                                attachments_with_sinopsys,
                            )
                        )
            ...
            print(ex)
            return HttpResponseBadRequest(
                "El id que solicitaste tiene un formato erroneo"
            )
            ...
            print(ex)
            return HttpResponseServerError(
                "El servidor tardo en retornar una respuesta"
            )
            ...
            print(ex)
            return HttpResponseServerError(
                "El servidor fallo en la ejecución de la operación"
            )
...
def get_pdf_of_dof_document(request, id):
    if request.method != "GET":
        ...
        try:
            ...
            return JsonResponse(
                {"error": "El documento no se puede convertir a pdf"}, status=500
            )
        except ValueError:
            return JsonResponse(
                {"error": "El documento no se puede convertir a pdf"}, status=500
            ...
            return JsonResponse(
                {"error": "Documento con identificador invalido"}, status=400
            )
        except ServerSelectionTimeoutError:
            return JsonResponse(
                {"error": "El servidor tardo en retornar una respuesta"}, status=500
            )
        except ConnectionFailure:
            return JsonResponse(
                {"error": "Error en la conexión a la base de datos"}, status=500
            )
        except OperationFailure:
            return JsonResponse(
                {"error": "El servidor fallo en la ejecución de la operación"},
                status=500,
            )
...
def get_doc_sinopsys_from_dof_collection(request, id):
    if request.method != "GET":
        ...
        try:
            ...
            sinopsys = search_result_repo.get_doc_sinopsys_from_dof_collection(id)
            if sinopsys:
                keyword_id = int(request.GET.get("keyword"))
                keyword = get_object_or_404(Keyword, pk=keyword_id)
                subkeywords = list(
...
def generate_pdf(request):
    if request.method != "POST":
        ...
        try:
            ...
            return JsonResponse(
                {"error": "No se pudieron obtener los IDs seleccionados"}, status=400
            )
        except InvalidId:
            return HttpResponseBadRequest(
                "El id que solicitaste tiene un formato erroneo"
...
def validate_data_to_generate_pdf(selected_ids: list, keyword: str):
    ...
    if len(selected_ids) > 10:
        return JsonResponse(
            {"error": "La cantidad máxima de documentos a exportar es 10"}, status=400
        )
    if not keyword:
        return JsonResponse(
            {"error": "Se necesita el parametro de keyword"}, status=400
        )
...
def send_search_results_mail(request, keyword_id, recipient_list):
    ...
    mail = create_mail(
        recipient,
        f"Resultados de búsqueda de Compass - Keyword: {keyword_title}",
        context,
        "mails/search_results.html",
        [pdf],
    )
